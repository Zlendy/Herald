...
impl AsyncFactoryComponent for ApplicationModel {
    type Init = ApplicationModel;
    type Input = ();
    ...
    fn output_to_parent_input(output: Self::Output) -> Option<FactorySignal> {
        Some(match output {
            ApplicationModelOutput::Remove(index) => FactorySignal::Remove(index),
        })
    }
...
pub struct AppFactory {
    default_widget: ApplicationModel,
    messages: AsyncFactoryVecDeque<ApplicationModel>,
}

#[derive(Debug)]
pub enum FactorySignal {
    PushDefault,
    PushBack(ApplicationModel),
    Remove(DynamicIndex),
    ClearLocalData,
    SetData,
}
...
impl AsyncComponent for AppFactory {
    type Init = ApplicationModel;
    ...
    async fn update(
        ...
        match msg {
            ...
            FactorySignal::SetData => {
                let Ok(apps) = GotifyService::instance().get_apps().await else {
                    return;
                };
                ...
                for app in apps {
                    log::debug!("{:#?}", app);
                    sender.input(FactorySignal::PushBack(app));
                }
            }
        }
    }
