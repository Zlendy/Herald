...
impl AsyncFactoryComponent for UserModel {
    type Init = UserModel;
    type Input = ();
    ...
    view! {
        root = adw::PreferencesGroup {
            ...
            adw::EntryRow {
                set_title: "Admin",
                set_text: if self.admin { "Yes" } else { "No" },
                set_editable: false,
            },
    ...
    fn output_to_parent_input(output: Self::Output) -> Option<FactorySignal> {
        Some(match output {
            UserModelOutput::Remove(index) => FactorySignal::Remove(index),
        })
    }
...
pub struct UserFactory {
    default_widget: UserModel,
    messages: AsyncFactoryVecDeque<UserModel>,
}
...
impl AsyncComponent for UserFactory {
    type Init = UserModel;
    ...
    async fn update(
        ...
        match msg {
            ...
            FactorySignal::SetData => {
                let Ok(users) = GotifyService::instance().get_users().await else {
                    return;
                };
                ...
                for user in users {
                    log::debug!("{:#?}", user);
                    sender.input(FactorySignal::PushBack(user));
                }
            }
        }
    }
